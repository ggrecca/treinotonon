-- Patch de publicação para bancos que já receberam as migrations 001-003.
-- Corrige imutabilidade de propriedade, compatibilidade de conjugados legados
-- e atualiza a RPC de treino. A prescrição de cargas por série usa as colunas
-- target_load já existentes em prescribed_sets e prescribed_drops.

-- Patch idempotente para bancos que já receberam a versão original de 202607110001.
-- Pode ser executado com segurança mesmo em instalação nova.
...
-- Métodos legados de conjugados agora são representados pelas colunas próprias.
update public.workout_exercises
set method = 'normal'
where conjugate_block_id is not null
  and method in ('bi_set', 'tri_set');
...
declare
  ...
  v_existing_source_workout_id uuid;
...
begin
  ...
  select w.type, w.owner_id, w.coach_id, w.student_id, w.source_workout_id, true
  into v_existing_type, v_existing_owner_id, v_existing_coach_id, v_existing_student_id, v_existing_source_workout_id, v_exists
  ...
  if v_exists and (
    v_type is distinct from v_existing_type
    or v_owner_id is distinct from v_existing_owner_id
    or v_coach_id is distinct from v_existing_coach_id
    or v_student_id is distinct from v_existing_student_id
    or v_source_workout_id is distinct from v_existing_source_workout_id
  ) then
    raise exception using
      message = 'Não é permitido transferir ou mudar a origem de um treino existente. Crie uma cópia para outro aluno.',
      errcode = '55000';
  end if;
  ...
  select coalesce(jsonb_agg(
    jsonb_build_object(
      ...
      'type', case when nullif(trim(ex.value ->> 'conjugate_block_id'), '') is not null and coalesce(nullif(trim(ex.value ->> 'method'), ''), 'normal') in ('bi_set','tri_set') then 'normal' else coalesce(nullif(trim(ex.value ->> 'method'), ''), 'normal') end,
    ...
    if v_has_history then
      select coalesce(jsonb_agg(
        jsonb_build_object(
          ...
          'type', case when we.conjugate_block_id is not null and coalesce(nullif(trim(we.method), ''), 'normal') in ('bi_set','tri_set') then 'normal' else coalesce(nullif(trim(we.method), ''), 'normal') end,
